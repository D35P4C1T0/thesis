...
L'idea di fondo dell'intero progetto è quella di creare un sistema riutilizzabile e facilmente integrabile in altri progetti. A questo scopo, la funzione \texttt{register\_heatmap()}, già apparsa nel frammento di codice nel Listing \ref{lst:register_hm}, offre un'interfaccia semplificata che permette di integrare il progetto in qualsiasi applicazione Flask già esistente, senza dover modificare nulla a livello di struttura.
...
Per esempio, nel Listing \ref{lst:register_heatmap_def} si può notare come venga creata una Blueprint a partire da una configurazione data. In particolare, viene chiamata la funzione \texttt{create\_heatmap\_blueprint()} (riga 12); tale funzione accetta una configurazione (\texttt{HeatmapConfig} o \texttt{dict}) e restituisce un \textit{blueprint} Flask configurato per la visualizzazione delle heatmap. 
Al suo interno viene creato un oggetto \texttt{Blueprint}; i parametri necessari per la creazione di una Blueprint sono illustrati nel Listing \ref{lst:create_heatmap_blueprint}. All'interno dello stesso file vengono dichiarate anche le rotte del blueprint. 
...
Dal punto di vista dell'utente, la rotta coincide con l'URL digitato nella barra degli indirizzi o richiamato da un'applicazione. Tuttavia, tecnicamente, la rotta non punta a un file fisico sul server (come avveniva nel vecchio web statico con i file \texttt{.html}), bensì a un endpoint. È un \say{segnapolo} logico che istruisce il framework (Flask, in questo caso) su quale specifica porzione di codice deve "rispondere" a una determinata chiamata.
